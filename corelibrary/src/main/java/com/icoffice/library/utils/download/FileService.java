...
public class FileService {
	...
	public void save(String path, Map<Integer, Integer> map) {// int threadid,
		...
		db.close();
		CommonUtils.showLog(CommonUtils.UPGRADE_TAG,"db save " + path );
	}
	...
	public void update(String path, int threadId, int pos) {
		...
		db.close();
		CommonUtils.showLog(CommonUtils.UPGRADE_TAG,"db update " + path + " " + threadId + " " + pos);
	}
	...
	public void delete(String path) {
		...
		db.execSQL("delete from filedownlog where downpath=?",
				new Object[] { path });
		db.close();
		CommonUtils.showLog(CommonUtils.UPGRADE_TAG,"db delete " + path);
	}
